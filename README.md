...
回归测试在 `tests\`（34 项，pwsh 与 Windows PowerShell 5.1 双宿主），CI 每次 push/PR 自动运行。
...
1. 校验输入版本 == 根目录 `VERSION`，然后跑全部回归测试（34 项，PowerShell 7 + 5.1）
...
├── tests/                  # 回归测试（34 项）：安装所有权 / 卸载守卫 / 账本正则 / 版本门槛 /
