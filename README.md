...
- **安装目录所有权**：`.dsh-desktop-shell-root` 标记；非空且非本产品目录拒绝安装；Preflight→Stage→Initialize→Commit 事务式安装，失败不留半安装；卸载前再次验证
...
- **发布链**：一键安装对 Release 资产做 SHA256 完整性校验（防下载损坏/资产错配）；插件推荐全部锁定版本
