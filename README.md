...
> DesktopShell v1.0.11（DSH `0.1.5-rc.2` 核心基线；发布状态以 GitHub Release 为准） · DSH `0.1.5-rc.2`（默认；最低兼容版本为 rc.7；rc.7 / rc.8 / 0.1.1 rc.1 / rc.2 / 0.1.5 rc.1 / rc.2 已列入测试基线）；未来 DSH 按 CLI 能力 best-effort 兼容
...
irm https://raw.githubusercontent.com/metahumanz/DeepSeekHarness-DesktopShell/v1.0.11/scripts/Install-FromGitHub.ps1 -OutFile "$env:TEMP\install-dsh.ps1"
& "$env:TEMP\install-dsh.ps1" -Owner metahumanz -Repo DeepSeekHarness-DesktopShell -Tag v1.0.11
```
...
GitHub Actions → **Release → Run workflow**，输入版本号（如 `1.0.11`，必须与根目录
