...
> `dsh-dream-skin@^0.4.10`（含 sticky restore 加固与 host-backed 持久化）。本检查表**必须在本机真实
> Windows 桌面完成**——托盘、WebView2、连续重启、皮肤恢复属于 GUI 行为，源码级测试不能替代人工验收。
> v1.0.4 曾使用 `^0.4.1`，仅为历史发布基线；以下检查表按当前目录的 Dream Skin spec 执行。
> 未取得用户明确 25 次验收记录前，
> 不得声称“验收通过”。
...
2. 管理器 → 安装插件 → 选 **22. Dream Skin 主题**（`dsh-dream-skin@^0.4.10`）。
   若 Profile 里已有旧 0.3.0 实现，应看到升级确认提示：
   `检测到 Dream Skin 0.3.0 旧实现……是否升级到 dsh-dream-skin@^0.4.10？` → 选是。
