...
- 端口归属只接受回环 DSH 命令行特征，且在指定端口时要求完整匹配 `--port <端口>`，避免把
  `30801` 误认为 `3080`。
- Cost Meter 账本修复会先限制输入大小（最多 16 Mi 字符），再解析和原子写入；异常或超限文件保持原样。

## 插件目录与可复现性

管理器当前提供 26 项可移植推荐插件。`Installed` 字段只是本机 Profile 的版本快照，表示“曾安装”，
不等同于 BootReady 兼容 PASS。

- 精确 npm 版本和带版本的 npm spec 由目录明确声明。
- npm range 会按包管理器规则解析兼容版本，不是不可变锁定。
- 8 个 GitHub spec 没有 tag 或 commit，目录会明确标作“GitHub 浮动引用”；安装时会解析上游默认分支，
  不应被理解为固定兼容版本。
