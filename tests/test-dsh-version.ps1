...
$names = @('ConvertTo-SemVerParts', 'Compare-DshVersion', 'Test-DshVersionSupported', 'Test-DshVersionTested')
$funcs = $ast.FindAll({
    param($node)
...
# 兼容策略 = 最低兼容版本 + 测试版本：
# - 低于 minimumCompatibleDshVersion → 不支持
# - 等于/高于 minimum（含 rc.7/rc.8/未来版本）→ 允许尝试
# - 无法解析 → 无法证明达到最低版本，按不支持
# - 空版本串（无法读取）→ $null（未知）
...
Assert-Equal "support 0.1.5 rc2"           (Test-DshVersionSupported '0.1.5-rc.2') $true
...
Assert-Equal "tested 0.1.5 rc2"           (Test-DshVersionTested '0.1.5-rc.2') $true
